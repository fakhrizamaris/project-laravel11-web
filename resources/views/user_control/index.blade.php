                            ...
                            @foreach ($uc as $item)
                                <tbody>
                                    ...
                                    @if ($item->role === 'superadmin')
                                        <td style="color:rgb(255, 140, 0); font-weight: bold;">
                                            {{ $item->role }}</td>
                                    ...
                                    <td>{{ $item->email }}</td>
                                    @if ($item->role === 'superadmin')
                                        <td style="color:rgb(255, 140, 0); font-weight: bold;">Super Admin</td>
                                    @elseif ($item->role === 'admin')
                                        <td>
                                            {{-- <form
                                                onsubmit="return confirmUser('Yakin ingin Mengangkat USER Menjadi ADMIN ?')"
                                            ...
                                            <form onsubmit="return confirmAdmin('Yakin ingin menurunkan ADMIN ?')"
                                                class="d-inline" action="/downrole/{{ $item->id }}" method="POST">
                                                @csrf
                                                <input type="submit"
                                                    class="btn-sm text-decoration-none border border-warning text-warning"
                                                    value="DOWN">
                                    ...
                                    @else
                                        <td>
                                            <form
                                                onsubmit="return confirmUser('Yakin ingin mengangkat user ini menjadi ADMIN?')"
                                                class="d-inline" action="/uprole/{{ $item->id }}" method="POST">
                                                @csrf
                                                <input type="submit"
                                                    class="btn-sm text-decoration-none border border-warning text-warning"
...
<script>
    ...
    //             // Jika tombol konfirmasi diklik, kirim formulir
    //             document.querySelector('form[action*="/uprole-form/{{ $item->id }}"]').submit();
    //         }
    //     });
    //     // Mencegah formulir dikirimkan secara otomatis
    //     return false;
    // }
    ...
    //         if (result.isConfirmed) {
    //             // Jika tombol konfirmasi diklik, kirim formulir
    //             document.querySelector('form[action*="/downrole/{{ $item->id }}"]').submit();
    //         }
    //     });
    //     // Mencegah formulir dikirimkan secara otomatis
    //     return false;
